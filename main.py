...
accounts = {
    "4356917206": "FD5364ADEF5CABF22B54D82235F5572C2AF42B65EC799CEC90FD9E4B3E32A318"
}
...
def home():
    response_data = {
        ...
        "API_Usage_Guide": {
            "API_Format": {
                "Deploy_Ghost": "/ghost?teamcode={teamcode}&ghost_names={ghostnames}&ghost={1-5}",
                "Leave_Squad": "/leave"
            },
            "Examples": {
                "Example_Deploy_1": "/ghost?teamcode=1234567&ghost_names=RiduanCodex&ghost=1",
                "Example_Deploy_Multi": "/ghost?teamcode=1234567&ghost_names=RiduanCodex1,RiduanCodex2,RiduanCodex3&ghost=3",
                "Example_Leave": "/leave"
            }
        }
    }
...
def ghost():
    ...
    ghost_names_param = request.args.get("ghost_names", "Ghost")
    ghost_count_param = request.args.get("ghost", "1")
    ...
    ghost_names_list = [name.strip() for name in ghost_names_param.split(",") if name.strip()]
    if not ghost_names_list:
        ghost_names_list = ["Ghost"]
    ...
    def deploy_bot(client_data):
        ...
        name = ghost_names_list[idx] if idx < len(ghost_names_list) else ghost_names_list[-1]
        result = client.execute_command("/XRRR", teamcode, name)
        return account_id, f"{result} | Name: {name}"
...
if os.getenv("VERCEL") != "1" and os.getenv("RENDER") != "true":
    try:
